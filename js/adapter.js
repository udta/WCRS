...
(function() {
  ...
  if (browserDetails.version < browserDetails.minVersion) {
    console.log('Browser: ' + browserDetails.browser + ' Version: ' +
  ...
  var ieShim = require('./ie/ie_shim') || null;
  var safariShim = require('./ie/ie_shim') || null; //For Safari without WebRTC support, version <= 9.1
  ...
  case 'opera':
    ...
    case 'ie':
    case 'safari':
      if (!ieShim||!ieShim.shimPeerConnection) {
          logging('IE / Safari shim is not included in this adapter release.');
          return;
        }
      ...
      if (browserDetails.browser == "ie") {
        ...
        // init
        var result = { installed : false, version : undefined };
        browserDetails.isSupportWebRTC = false;
        browserDetails.isSupportORTC = false;
        ...
        // Export to the adapter global object visible in the browser.
        module.exports.browserShim = ieShim;

        //set function handlers
        navigator.getUserMedia = ieShim.shimGetUserMedia;
        //ieShim.shimSourceObject();
        window.RTCPeerConnection = ieShim.shimPeerConnection;
        window.RTCIceCandidate = ieShim.shimRTCIceCandidate;
        window.RTCSessionDescription = ieShim.shimRTCSessionDescription;
        window.attachMediaStream = ieShim.attachMediaStream;
        window.loadWindows = ieShim.loadWindows;
...
},{"./chrome/chrome_shim":2,"./edge/edge_shim":4,"./firefox/firefox_shim":5,"./utils":6,"./ie/ie_shim":7}],2:[function(require,module,exports){
/*
 *  Copyright (c) 2016 The WebRTC project authors. All Rights Reserved.
 *
 *  Use of this source code is governed by a BSD-style license
...
// Parses ICE candidate from SDP media section.
SDPUtils.getIceCandidates = function(mediaSection) {
   var lines = SDPUtils.splitLines(mediaSection);
   lines = lines.filter( function (line) {
       return line.match("a=candidate");
   }) ;
   return lines;
};
  ...
  shimPeerConnection: function() {
    ...
    window.RTCPeerConnection = function(config) {
      ...
      if (config && config.iceServers) {
        ...
        //    server.urls = server.urls.split("").filter(function(url) {
        //      return url.indexOf('transport=udp') !== -1;
        //    })[0];
            //return !server.urls.match(/^stun:/);
        //    return true;
         // }
         // return false;
       // });
       this.iceOptions.iceServers = [];
       //this.iceOptions.iceServers = [{ "urls": "turn:turn-testdrive.cloudapp.net:3478?transport=udp", "username": "redmond", "credential": "redmond123" }];
      }
      ...
      this._localIceCandidatesBuffer.forEach(function(event) {
        if (self.onicecandidate !== null) {
          ...
          if (event && event.candidate && event.candidate.candidate ) {
              if ( -1 === event.candidate.candidate.indexOf("endOfCandidates") ) {
                  //self.localDescription.sdp += 'a=' + event.candidate.candidate + '\r\n';
                  var index = self.localDescription.sdp.indexOf("a=rtpmap:"); 
                  var part_1 = self.localDescription.sdp.substring(0, index);
                  var part_2 = self.localDescription.sdp.substring(index);
                  self.localDescription.sdp = part_1 + 'a=' + event.candidate.candidate + '\r\n' + part_2;
              } else {
                  /*Update the Address ON the C & M lines*/
                  var address = self._localIceCandidatesBuffer[0].candidate.candidate.split(" ")[4];
                  self.localDescription.sdp = self.localDescription.sdp.replace(/0.0.0.0/g, address );  
                  
                  /*Update the Port ON the C & M ilnes  ( FOR AUIDO OLNY  )*/
                  var audio_port = "m=audio "+self._localIceCandidatesBuffer[0].candidate.candidate.split(" ")[5];
                  self.localDescription.sdp = self.localDescription.sdp.replace(/m=audio [0-9]*/g, audio_port );

                  /*Update the Port ON the C & M ilnes  ( FOR VIDEO OLNY  )*/
                  var video_port = "m=video "+self._localIceCandidatesBuffer[0].candidate.candidate.split(" ")[5];
                  self.localDescription.sdp = self.localDescription.sdp.replace(/m=video [0-9]*/g, video_port );
              } 
          }
    ...
    window.RTCPeerConnection.prototype._createIceAndDtlsTransports =
      ...
      iceGatherer.onlocalcandidate = function(evt) {
        ...
        var complete = iceGatherer.state === 'complete' ? true : false; 
        ...
        if (self.onicecandidate !== null) {
          ...
          if (self.localDescription && self.localDescription.type === '') {
            ...
            self.iceGatheringState = 'gathering';
            //if (event && !event.candidate.candidate.match("endOfCandidates")) {
            //    self.localDescription.sdp += 'a=' + event.candidate.candidate + '\r\n';
            //}
            self.onicecandidate(event);
            if (complete) {
      ...
      iceTransport.onicestatechange = function(evt) {
        console.error("ICE statechanged!!! %s", evt.state);
        self._updateConnectionState();
      };

      var dtlsTransport = new RTCDtlsTransport(iceTransport);
      dtlsTransport.ondtlsstatechange = function(evt) {
        console.error("DTLS statechanged!!! %s", evt.state);
        self._updateConnectionState();
      };
    ...
    window.RTCPeerConnection.prototype._transceive = function(transceiver,
      ...
      var params = this._getCommonCapabilities(transceiver.localCapabilities,
        ...
        /*Process when no common codec!*/
        if (params.codecs.length === 0) {
            console.error("No common codec !!!!!");
            return; 
        }
      ...
      if (recv && transceiver.rtpReceiver) {
        params.encodings = [{
          ssrc: transceiver.recvSsrc ? transceiver.recvSsrc : 9999, //RZHANG, for avoiding interface error on Edge.
          active: true
        }];
        params.rtcp = {
    ...
    window.RTCPeerConnection.prototype.setLocalDescription =
      ...
      if (description.type === 'offer') {
        ...
        sections.forEach(function(mediaSection, sdpMLineIndex) {
          ...
          var rejected = mediaSection.split('\n', 1)[0].split(' ', 2)[1] === '0'; //Judge the remote "m=" line port

          if (!rejected) {
              //Need  to judge the local codecs too
              var params = self._getCommonCapabilities(localCapabilities,
                remoteCapabilities);
              rejected = params.codecs.length === 0;
          }
          ...
          if (!rejected) {
            ...
            var remoteDtlsParameters = SDPUtils.getDtlsParameters(mediaSection,
              sessionpart);
            //To avoiding issue on Edge...
            remoteDtlsParameters.fingerprints.forEach( function(fingerprint){
                fingerprint.algorithm = fingerprint.algorithm.toLowerCase();
            });
            /*HACK: rzhang, IPVT X SERVER will always be server*/
            ...
            self._transceive(transceiver,
              ...
              //Process the iceTransport and dtlsTransport
              dtlsTransport.ondtlsstatechange = null;
          }
        });
      }
    ...
    window.RTCPeerConnection.prototype.setRemoteDescription =
      ...
      sections.forEach(function(mediaSection, sdpMLineIndex) {
        ...
        if (!rejected) {
          var remoteCandidates = SDPUtils.getIceCandidates(mediaSection);
          //remoteCandidates.forEach( function (line) {
          //    self.addIceCandidate(SDPUtils.parseCandidate(line));
          //} );
        ...
        var mid_section = SDPUtils.matchPrefix(mediaSection, 'a=mid:')[0];
        var mid = null
        if ( mid_section ) {
            mid = mid_section.substr(6);
        }
        ...
        if (description.type === 'offer') {
          ...
          rtpReceiver.onerror = function(e) {
          ...
          var remoteC = [];
          remoteCandidates.forEach( function (line) {
              var candidate = SDPUtils.parseCandidate(line);
              //iceTransport.addRemoteCandidate(candidate);
              remoteC.push(candidate);
           } );
          //Marked remoteCandidate complete
          //var candidateComplete = {};
          //candidateComplete.complete = true;
          //iceTransport.addRemoteCandidate(candidateComplete);
          iceTransport.setRemoteCandidates(remoteC);  
          //iceGatherer.state = undefined;
  ...
  detectBrowser: function() {
    ...
    if (navigator.mediaDevices &&
      ...
      result.version = this.extractVersion(navigator.userAgent,
          /Edge\/(\d+).(\d+)$/, 1);
      //result.minVersion = 10586; //We need EdgeHTML version 13 upper
      result.minVersion = 13;
      return result;
    }
    
    // IE
    if ( !navigator.mediaDevices && ( !!window.ActiveXObject || 'ActiveXObject' in window  ||
        navigator.userAgent.match(/MSIE (\d+)/) || navigator.userAgent.match(/rv:(\d+)/) ) ) {
      result.browser = 'ie';
      result.version = this.extractVersion(navigator.userAgent,
          /MSIE (\d+).(\d+)/, 1);
    ...
    // Chrome/Chromium/Webview. OR Opera
    ...
    if ( (!navigator.webkitGetUserMedia && navigator.userAgent.match(/AppleWebKit\/([0-9]+)\./) ) 
    ...
    //Safari with full webRTC support
    //We will wait for this Safari!!
            ...
            if (!element.pluginObj && stream) {
             ...
             if (element.pluginObj) {
               ...
               // });
                ...
                element.pluginObj.onplaying = function(e) {
                    logging("Object Event: playing ~~~~~");
                };

                //element.pluginObj.addEventListener('waiting', function(objvid) {
                //    logging("Objcet Event: waiting" );
                //});
                
                element.pluginObj.onwaiting = function(e) {
                    logging("Object Event: waiting ~~~~~");
                };

                //element.pluginObj.addEventListener('progress', function(objvid) {
                //    logging("Objcet Event: progress" );
                //});

                element.pluginObj.onprogress = function(e) {
                    logging("Object Event: progress ~~~~~");
                };

                element.pluginObj.onstalled = function(e) {
                    logging("Object Event: stalled ~~~~~");
                };
                element.pluginObj.onsuspend = function(e) {
                    logging("Object Event: suspend ~~~~~");
                };
                ...
                //element.pluginObj.addEventListener('timeupdate', function(objvid) {
                //    logging("Objcet Event: timeupdate" );
                //});

                //element.pluginObj.addEventListener('volumechange', function(objvid) {
                //    logging("Objcet Event: volumechange" );
                //});
...
// Expose public methods.
module.exports = {
  shimOnTrack: ieShim.shimOnTrack,
  shimSourceObject: ieShim.shimSourceObject,
  shimPeerConnection: ieShim.shimRTCPeerConnection,
  shimGetUserMedia: ieShim.shimGetUserMedia,
